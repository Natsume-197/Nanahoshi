...
export function mountSeriesShareImages(app: Hono) {
	app.get("/api/share/collections/:filename", async (c) => {
		...
		try {
			const preview = await getCollectionSharePreview(uuid);
			...
			log.error({ err, uuid }, "Error creating collection share image");
