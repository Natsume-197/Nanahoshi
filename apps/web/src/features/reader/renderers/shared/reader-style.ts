...
export function buildReaderStyle(c: ReaderStyleConfig): CSSProperties {
	// nanahoshi: vertical-only font features, joined like its fontFeatureSettings.
...
export function buildReaderClasses(c: ReaderClassesConfig): string {
	return [
		...
		c.hideFurigana
			...
			: "",
		c.fontWeight ? "nanahoshi-apply-font-weight" : "",
		c.prioritizeReaderStyles ? "nanahoshi-apply-important" : "",
		c.enableTextJustification ? "nanahoshi-apply-justification" : "",
		c.enableTextWrapPretty ? "nanahoshi-text-wrap-pretty" : "",
		c.textMarginMode === "manual" ? "nanahoshi-margin-manual" : "",
	]
		.filter(Boolean)
		.join(" ");
}
