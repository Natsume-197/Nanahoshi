...
describe("PDF view state", () => {
	test("stores clamped, one-based page positions", () => {
		...
		expect(positionForPdfPage(2, 3, createPdfSections(3)).locator).toEqual({
			sectionReference: "pdf-page-2",
			characterOffset: 0,
		});
	});
	...
	test("keeps the cover alone and pairs the remaining spread pages", () => {
		expect(pdfPagesForLayout(0, 6, "spread-even")).toEqual([0]);
		expect(pdfPagesForLayout(1, 6, "spread-even")).toEqual([1, 2]);
		expect(pdfPagesForLayout(2, 6, "spread-odd")).toEqual([1, 2]);
		expect(pdfPagesForLayout(5, 6, "spread-even")).toEqual([5]);
