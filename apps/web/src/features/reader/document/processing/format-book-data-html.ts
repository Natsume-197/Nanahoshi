...
} from "@/features/reader/document/processing/image-dimensions";
 ...
 *  GIF data URI (`data:image/gif;nanahoshi:<key>;base64,…`) and a bare `nanahoshi:<key>`. */
const IMAGE_REF_PREFIX_RE = /(?:data:image\/gif;)?nanahoshi:/g;
...
function markContainFit(el: HTMLElement, matchers: AxisPinnedMatchers) {
	for (const imgEl of Array.from(el.getElementsByTagName("img"))) {
		...
		imgEl.toggleAttribute("data-nanahoshi-contain-fit", true);
	}
}
...
async function reserveImageDimensions(
	...
	await Promise.all(
		Array.from(el.getElementsByTagName("img")).map(async (imgEl) => {
			...
			imgEl.setAttribute(
				"data-nanahoshi-natural-width",
				String(dimensions.width),
			);
			imgEl.setAttribute(
				"data-nanahoshi-natural-height",
...
function addImageContainerClass(el: HTMLElement) {
	for (const imgEl of Array.from(el.getElementsByTagName("img"))) {
		...
		parentEl?.classList.add("nanahoshi-img-container");

		if (!isElementGaiji(imgEl)) {
			parentEl?.classList.add("nanahoshi-illustration-container");
		}
	}
}
 ...
 * the continuous reader's auto-height flex chain (visual mode caps it via its own
...
function wrapImages(el: HTMLElement, document: Document) {
	for (const childNode of el.children) {
		for (const tag of Array.from(childNode.getElementsByTagName("img")).filter(
			...
			wrapper.classList.add("nanahoshi-img-parent");
