...
export function refitImageWidths(root: HTMLElement, fitHeight: number): void {
	for (const imgEl of Array.from(
		root.querySelectorAll("img[data-nanahoshi-natural-width]"),
	)) {
		const width = Number(imgEl.getAttribute("data-nanahoshi-natural-width"));
		const height = Number(imgEl.getAttribute("data-nanahoshi-natural-height"));
