...
const loadingScreen = readFileSync(
	new URL("./reader-loading-screen.tsx", import.meta.url),
	"utf8",
);
...
describe("reader entry motion", () => {
	it("shows the reader shell without the router's default pending delay", () => {
		...
		expect(readerRoute).toContain("<ReaderLoadingScreen");
	});

	it("fades the routed surface without containing its fixed reader chrome", () => {
		expect(readerRoute).toContain('className="reader-route-content ');
		expect(readerRoute).toContain("font-reader-sans");
