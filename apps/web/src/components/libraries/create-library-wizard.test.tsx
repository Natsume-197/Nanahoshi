...
test("opening sources with server-disabled providers stays clean, but real edits are dirty", {
	timeout: 20000,
}, async () => {
	providerAvailability.hardcover = false;
	const { MetadataSection } = await import("./library-detail/metadata-section");
	...
	await waitFor(() => expect(onDirtyChange.mock.calls.at(-1)?.[0]).toBe(true));
	fireEvent.click(
		view.getByRole("button", { name: m["library.rules_discard"]() }),
	);
	await waitFor(() => expect(onDirtyChange.mock.calls.at(-1)?.[0]).toBe(false));
	expect(view.queryByText(m["library.rules_unsaved"]())).toBeNull();
});
