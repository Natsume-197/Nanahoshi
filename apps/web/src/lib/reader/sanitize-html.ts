 ...
 * everything the ttu-parity reader relies on — HTML + SVG (cover images),
 ...
 *
 * Packed images are still `ttu:`/dummy-data-URI references at this point (the
 * blob: object URLs only exist per-open), so the internal `ttu:` scheme is
...
export function sanitizeStoredBookHtml(element: HTMLElement): HTMLElement {
	...
	DOMPurify.sanitize(element, {
		...CONFIG,
		ALLOWED_URI_REGEXP:
			/^(?:(?:(?:f|ht)tps?|mailto|tel|callto|cid|xmpp|data|blob|ttu):|[^a-z]|[a-z+.-]+(?:[^a-z+.\-:]|$))/i,
		IN_PLACE: true,
	});
	return element;
}
