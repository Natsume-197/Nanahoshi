 ...
 * All rights reserved.
 *
 * Based on the css parser/compiler by NxtChg
 * https://github.com/NxtChg/pieces/tree/master/js/css_parser
 *
 * Typed and reduced port: only top-level plain rules are kept, since the
 * style sheet formatter discards at-rules anyway (same output as ttu's
 * parse + filter pipeline). Lenient on malformed CSS instead of throwing.
 */
