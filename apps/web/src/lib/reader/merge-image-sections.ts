/**
 * Groups runs of consecutive image-only sections (each illustration/manga
...
export function mergeImageOnlySectionRuns(
	...
	// this targets are manga pages, which always carry reserved dimensions.
	const isLandscape = (el: Element) => {
		const img = el.querySelector("img, svg image");
		if (!img) return false;
		const width = Number(
			img.getAttribute("data-ttu-natural-width") ?? img.getAttribute("width"),
		);
		const height = Number(
			img.getAttribute("data-ttu-natural-height") ?? img.getAttribute("height"),
		);
		return width > 0 && height > 0 && width > height;
	};

	const isImageOnly = (el: Element) =>
		el.querySelector(":scope > .ttu-no-text") !== null &&
