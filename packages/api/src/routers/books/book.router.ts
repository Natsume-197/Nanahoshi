...
export const bookRouter = {
	...
	// Manual fix-match: which provider tabs to offer for this book's library
	// (enabled + credential present). Unconfigured providers (Comicvine/Hardcover
	// without a key) would only ever return empty results.
	availableMetadataProviders: protectedProcedure
		...
		.handler(async ({ input, context }) => {
			...
			return bookMetadataService.getAvailableProviders(serverId, book.id);
		}),
