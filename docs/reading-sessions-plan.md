...
Las sesiones aparecen en una tabla semántica con grupos por día, cabecera fija y desplazamiento interno para conservar las columnas en pantallas estrechas. El resumen, récords y lecturas anteriores ocupan un panel lateral cuando hay espacio; la gráfica queda debajo. La gráfica muestra duración por defecto, conserva la distancia temporal entre días y permite consultar cada punto con ratón o teclado; la vista de posición rompe la línea cuando faltan datos. `ReadingHistory` reinicia su estado al cambiar de libro. El historial vacío permite registrar la primera sesión y el filtro sin resultados ofrece mostrar toda la lectura.
